...
#API to get all  available pizzas
...
def checkout_cart(payment_type:str,delivery_address:str,db:Session=Depends(get_db),current_user:schemas.User=Depends(get_current_user)):
    ...
    #Assigning delivery partner for order if available
    ...
    #Creating payment details into payment table
    payment=models.Payment(order_id=order.id,price=total,payment_type=payment_type)
    db.add(payment)

    #Clearing all cart items from cart after checkout
...
def get_orders(db:Session=Depends(get_db),current_user:schemas.User=Depends(get_current_user)):
    ...
    #fetching orders list based on my userid
...
def get_payment_details(order_id:int,db:Session=Depends(get_db),current_user:schemas.User=Depends(get_current_user)):
    ...
    if not order:
        raise HTTPException(status_code=404,detail="Order not found for given user")
    
    payment=db.query(models.Payment).filter(models.Payment.order_id==order_id).first()
    if not payment:
        HTTPException(status_code=404,detail="payment details not found for given order id")
    return payment